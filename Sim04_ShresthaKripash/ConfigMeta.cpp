...
Config::Config()
{
	...
	Config_check.push_back("Processor Quantum Number:");
	Config_check.push_back("CPU Scheduling Code:");
	...
	Config_check.push_back("System memory ");
	Config_check.push_back("Memory block size ");
	Config_check.push_back("Projector quantity:");
	Config_check.push_back("Hard drive quantity:");
...
void Config::separateConfig(vector <string> &check, vector <int> &data, vector <string> &StringData)
{
	...
	for(counter = 1; counter < check.size() - 1; counter++)
	{
		...
		else if(counter == 2 || counter == 4 || counter == 16 || counter == 17)
		{
			check[counter] = first_half; 
			StringData.push_back(second_half);
		}	
		//the third being ints, which is converted using string to int function in the c++ library
		else if(counter == 3 || (counter >= 5 && counter <= 11) || counter == 14 || counter == 15)
		{
			data_hold = stoi(second_half, &convert);
			data.push_back(data_hold);
			check[counter] = first_half;
		}
		//the last being the memory for the system
		else if(counter == 12 || counter == 13)
		{
			...
			position = temp.find("{") + 1;
			position_check(position, "Config");			
			memory = temp.substr(position, first_half.length() - position - 2);
			first_half = temp.substr(0, position - 1);
			check[counter] = first_half;
			if(memory == "kbytes" || memory == "Mbytes"  || memory == "Gbytes")
			{

				if(counter == 12)
				{
					sys_memory = data_hold;
					convertMemory(memory);
				}
				if(counter == 13)
				{
					memory_block = data_hold;
					convertBlock(memory);
				}
			}
			else{
					error_handle("Invalid Memory in config file!");
			}
		}
	}
}
...
void Config::DataStorage(vector <int> &data, vector <string> &StringData)
{
	...
	Scheduler = StringData[1];
	Log_to = StringData[2];
	Log_file_path = StringData[3];

	QuantumNumber = data[0];
	Config_Data.insert(pair<string,int>("Quantum",data[0]));

	Processor_cycle_time = data[1];
	Config_Data.insert(pair<string,int>("run",data[1]));

	Monitor_display_time = data[2];
	Config_Data.insert(pair<string,int>("monitor",data[2]));

	Hard_drive_cycle_time = data[3];
	Config_Data.insert(pair<string,int>("hard drive",data[3]));	

	Project_cycle_time = data[4];
	Config_Data.insert(pair<string,int>("projector",data[4]));

	Keyboard_cycle_time = data[5];
	Config_Data.insert(pair<string,int>("keyboard",data[5]));

	Memory_cycle_time = data[6];
	Config_Data.insert(pair<string,int>("block",data[6]));
	Config_Data.insert(pair<string,int>("allocate",data[6]));

	Scanner_cycle_time = data[7];
	Config_Data.insert(pair<string,int>("scanner",data[7]));

	Project_quantity = data[10];
	Config_Data.insert(pair<string,int>("projector quantity",data[10]));

	Hard_drive_quantity = data[11];
	Config_Data.insert(pair<string,int>("hard drive quantity",data[11]));

	Config_Data.insert(pair<string,int>("memory block", sys_memory));
	Config_Data.insert(pair<string,int>("memory",sys_memory));
...
*
* @Parameters: 
* vector<string> &check, which is the vector containing line by line data from the Meta Data File
...
void MetaData::separateMeta(vector <string> &check, vector<string> &MetaFile)
{
	...
	check[0]= stringify(check[0]);
	
	check[check.size() - 1] = stringify(check[(check.size()-1)]);
	...
	for(counter = 1; counter < check.size() - 1; counter++)
	{
		...
		if(counter == check.size() - 2)
		{
			...
			size_t position2 = temp.rfind(";") + 1;
			position_check(position, "Meta");
			if(position2 == 0)
			{
				end = temp.substr(0, position);
			} 
			else
			{
				position_check(position2, "Meta");
				end = temp.substr(position2, position);
			}
			end = stringify(end);
...
void MetaData::MetaCheck(vector <string> &check, vector <string> &MetaFile)
{
	...
	{	
		...
		temp = check[0];				
		check[0] = stringify(temp);		//removes whitespace from beginning and end of string.
	}
	...
	hold = stringify(hold); //removes whitestring from beginning and end of string
...
void MetaData::calculateMetaData(vector <string> &MetaFile, Config &OS_Config)
{
	...
	for(int i = 2; i < (MetaFile.size() - 1); i++)
	{
		temp = MetaFile[i];
...
/*
* This is the convertMemory function for the Config class.
*
* If the string is kbytes, the function does nothing since it does not need to be converted. 
* If the string is Mbytes, the function multiplies the memory by 1024 to convert to kilobytes.
* If the string is Gbytes, the function multilpies the memory by 1024 * 1024 to convert to kilobytes.
*
* @Parameters: 
* string mem, which is the string to see how to convert the memory.
*
* @Return:
* The function returns nothing but changes the memory if needed.
*
*/
void Config::convertMemory(string mem)
{
	...
	{
		 sys_memory = sys_memory * 1024 * 1024;
	}

}

/*
* This is the convertBlock function for the Config class.
*
* If the string is kbytes, the function does nothing since it does not need to be converted. 
* If the string is Mbytes, the function multiplies the memory by 1024 to convert to kilobytes.
* If the string is Gbytes, the function multilpies the memory by 1024 * 1024 to convert to kilobytes.
...
*
* @Return:
* The function returns nothing but changes the memory block to Kilobytes if needed.
*
*/
void Config::convertBlock(string mem)
{
	
	if(mem == "Mbytes")
	{
			memory_block = memory_block * 1024;
	}
	else if(mem == "Gbytes")
	{
		 memory_block = memory_block * 1024 * 1024;
	}
