...
void Config::DataStorage(vector <int> &data, vector <string> &StringData)
{
	...
	Monitor_display_time = data[1];
	Config_Data.insert(pair<string,int>("monitor",data[1]));

	Processor_cycle_time = data[0];
	Config_Data.insert(pair<string,int>("run",data[0]));

	Scanner_cycle_time = data[6];
	Config_Data.insert(pair<string,int>("scanner",data[6]));

	Hard_drive_cycle_time = data[2];
	Config_Data.insert(pair<string,int>("hard drive",data[2]));
	...
	Project_cycle_time = data[3];
	Config_Data.insert(pair<string,int>("projector",data[3]));

	Config_Data.insert(pair<string,int>("memory block", sys_memory));
	Config_Data.insert(pair<string,int>("memory",sys_memory));
		
	Project_quantity = data[9];
	Config_Data.insert(pair<string,int>("projector quantity",data[7]));

	Hard_drive_quantity = data[10];
	Config_Data.insert(pair<string,int>("hard drive quantity",data[8]));
}
